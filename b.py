...
while is_reading:
    try:
        ...
        if id_exists:
            ...
            # Move the entire document to 'diem-danh' collection
            if name:
                diem_danh_ref = db.collection('diem-danh').document()
                # diem_danh_ref.set(doc_data)
                diem_danh_ref.set({'id': doc_data.get('id', ''), 'text': doc_data.get('text', ''), 'name': doc_data.get('name', ''), 'timestamp': time.time()})
